...
object DataSources extends App {
  ...
  val carsDF = spark.read
    ...
    .option("mode", "failFast") // permissive (default)
...
// alternative reading with options map
  ...
  spark.read
    .schema(carsSchema)
    .option("dateFormat", "YYYY-MM-dd") // couple with schema
    .option("allowSingleQuotes", "true")
    .option("compression", "uncompressed") // bzip2, gzip, etc
  ...
  moviesDF.write
    .mode(SaveMode.Overwrite)
    .option("compression", "snappy") //but default is snappy
